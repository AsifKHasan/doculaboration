...
from doc.docx_helper import DocxHelper
...
class DocFromJson(object):
    ...
    def run(self, config_file, json_file=None, nesting_level=0):
        ...
        for json_file in config_service._json_list:
            ...
            config_service._output_docx_path = config_service._output_dir / f"{json_file}.docx"
            ...
            # tear down
            self.end_time = int(round(time.time() * 1000))
            info(msg=f"script took {(self.end_time - self.start_time)/1000} seconds", nesting_level=nesting_level)
