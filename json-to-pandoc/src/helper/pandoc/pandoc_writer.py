...
def render_content_in_doc1(doc, cell_data):
...
def insert_content1(data, doc, container_width, container=None, cell=None, repeat_rows=0):
...
def insert_content_into_doc1(data, doc, start_row, row_from, container_width):
    # the content is by default one row content and we are only interested in the first column value
    row_data = data['sheets'][0]['data'][0]['rowData']
    first_cell_data = row_data[row_from - (start_row + 1)]['values'][0]
...
def insert_content_as_table1(data, doc, start_row, start_col, row_from, row_to, container_width, container=None, cell=None, repeat_rows=0):
    start_time = int(round(time.time() * 1000))
    current_time = int(round(time.time() * 1000))
    last_time = current_time
